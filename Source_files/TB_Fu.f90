...
subroutine dHij_r_F(TB_Hamil, atoms, Scell, numpar, M_Vs, M_dVs, M_d2Vs, M_cos, F, dF)
   ...
   if (.not.allocated(dHijx_r_all)) allocate(dHijx_r_all(N,N), source = 0.0d0)
   if (.not.allocated(dHijy_r_all)) allocate(dHijy_r_all(N,N), source = 0.0d0)
   if (.not.allocated(dHijz_r_all)) allocate(dHijz_r_all(N,N), source = 0.0d0)
   if (.not.allocated(d2Hijx_r2_all)) allocate(d2Hijx_r2_all(N,N), source = 0.0d0)
   if (.not.allocated(d2Hijy_r2_all)) allocate(d2Hijy_r2_all(N,N), source = 0.0d0)
   if (.not.allocated(d2Hijz_r2_all)) allocate(d2Hijz_r2_all(N,N), source = 0.0d0)
   ...
   error_part = 'Error in dHij_r_F:'
   call do_MPI_Allreduce(numpar%MPI_param, trim(adjustl(error_part))//'F', F)    ! module "MPI_subroutines"
   call do_MPI_Allreduce(numpar%MPI_param, trim(adjustl(error_part))//'dF', dF)  ! module "MPI_subroutines"
...
!$omp end parallel
#endif
end subroutine dHij_r_F


!11111111111111111111111111111111111111111111111111111111111111111111111
! Derivative of the atomic TB Hamiltonian:
subroutine dHij_s_F(TB_Hamil, atoms, Scell, NSC, numpar, Aij, M_x1, M_xrr) ! attractive forces for atoms, module "TB_Hamiltonian"
   type(TB_H_Fu), dimension(:,:), intent(in) :: TB_Hamil   ! parameters of the Hamiltonian of TB
   type(Atom), dimension(:), intent(in) :: atoms	! array of atoms in the supercell
   ...
   call Construct_M_Vs_F(Scell, NSC, TB_Hamil, numpar, M_Vs, M_dVs) ! subroitine below
   ...
   if (.not.allocated(dHijx_s_all)) allocate(dHijx_s_all(nat4,nat4), source = 0.0d0)
   if (.not.allocated(dHijy_s_all)) allocate(dHijy_s_all(nat4,nat4), source = 0.0d0)
   if (.not.allocated(dHijz_s_all)) allocate(dHijz_s_all(nat4,nat4), source = 0.0d0)
   ! Do the cycle (parallel) calculations:
   do k = Nstart, Nend, N_incr  ! each process does its own part
   ...
   ! Collect information from all processes into the master process, and distribute the final arrays to all processes:
   error_part = 'Error in dHij_r_F:'
   call do_MPI_Allreduce(numpar%MPI_param, trim(adjustl(error_part))//'Eelectr_s', Eelectr_s)    ! module "MPI_subroutines"

   do k = 1,nat ! Save for all the atoms:
...
end subroutine dHij_s_F



subroutine Construct_M_Vs_F(Scell, NSC, TB_Hamil, numpar, M_Vs, M_dVs, M_d2Vs, M_cos)
   ...
   !---------------------------
   real(8) :: part1, part2, d_part1, d_part2, d2_part1, d2_part2
      ...
      do atom_2 = 1,m ! do only for atoms close to that one
         ...
         if (j .GT. 0) then
            ...
            do k = 1, N	! for all electron orbitals (s, px, py, pz):
               part1 = Vs(TB_Hamil(Scell(NSC)%MDatoms(i)%KOA, Scell(NSC)%MDatoms(j)%KOA), k, r) ! function below
               part2 = Vs(TB_Hamil(Scell(NSC)%MDatoms(j)%KOA, Scell(NSC)%MDatoms(i)%KOA), k, r) ! function below
               M_Vs(k,i,j) = part1*TB_Hamil(Scell(NSC)%MDatoms(i)%KOA, Scell(NSC)%MDatoms(j)%KOA)%C_a + part2*TB_Hamil(Scell(NSC)%MDatoms(j)%KOA, Scell(NSC)%MDatoms(i)%KOA)%C_a

               d_part1 = dVs(TB_Hamil(Scell(NSC)%MDatoms(i)%KOA, Scell(NSC)%MDatoms(j)%KOA), k, r, part1) ! function below
               d_part2 = dVs(TB_Hamil(Scell(NSC)%MDatoms(j)%KOA, Scell(NSC)%MDatoms(i)%KOA), k, r, part2) ! function below
               M_dVs(k,i,j) = d_part1*TB_Hamil(Scell(NSC)%MDatoms(i)%KOA, Scell(NSC)%MDatoms(j)%KOA)%C_a + d_part2*TB_Hamil(Scell(NSC)%MDatoms(j)%KOA, Scell(NSC)%MDatoms(i)%KOA)%C_a
            enddo

            ! Second derivatives of Vs:
            if (present(M_d2Vs)) then
               do k = 1, N	! all orbitals
                  d2_part1 = d2Vs( TB_Hamil(Scell(NSC)%MDatoms(i)%KOA, Scell(NSC)%MDatoms(j)%KOA), k, r, part1, d_part1 ) ! function below
                  d2_part2 = d2Vs( TB_Hamil(Scell(NSC)%MDatoms(j)%KOA, Scell(NSC)%MDatoms(i)%KOA), k, r, part2, d_part2 ) ! function below
                  M_d2Vs(k,i,j) =  d2_part1*TB_Hamil(Scell(NSC)%MDatoms(i)%KOA, Scell(NSC)%MDatoms(j)%KOA)%C_a + d2_part2*TB_Hamil(Scell(NSC)%MDatoms(j)%KOA, Scell(NSC)%MDatoms(i)%KOA)%C_a
               enddo ! k = 1, N
   ...
   enddo ! do i = 1,nat
   ! Collect information from all processes into the master process, and distribute the final arrays to all processes:
   error_part = 'Error in Construct_M_Vs_F:'
   ...
   if (present(M_d2Vs)) then
      call do_MPI_Allreduce(numpar%MPI_param, trim(adjustl(error_part))//'M_d2Vs', M_d2Vs) ! module "MPI_subroutines"
   endif
   if (present(M_cos)) then
...
#else    ! OpenMP to use instead
   !$omp PARALLEL DO private(i, m, atom_2, j, k, x, y, z, r, part1, part2, d_part1, d_part2, d2_part1, d2_part2)
   do i = 1,nat	! all atoms
      m => Scell(NSC)%Near_neighbor_size(i)
      do atom_2 = 1,m ! do only for atoms close to that one  
         j => Scell(NSC)%Near_neighbor_list(i,atom_2) ! this is the list of such close atoms
...
end subroutine Construct_M_Vs_F
...
subroutine dHamil_tot_r(dHijx, dHijy, dHijz,  d2Hijx, d2Hijy, d2Hijz, TB_Hamil, Scell, NSC, numpar, k, M_Vs, M_dVs, M_d2Vs, M_cos)
   ...
   type(TB_H_Fu), dimension(:,:), intent(in) :: TB_Hamil ! all tight binding parameters
   type(Super_cell), dimension(:), intent(in), target :: Scell  ! supercell with all the atoms as one object
   integer, intent(in) :: NSC ! number of supercell
   ...
   do i = 1,nat	! all atoms
      ...
      do atom_2 = 1,m ! do only for atoms close to that one  
         ...
         if (j .GT. 0) then
            ...
            call dHamilton_one_s(i, j, k, dHijx1, dHijy1, dHijz1, Scell, NSC, atom_2, M_x1, M_xrr, M_Vs, M_dVs)
...
! See H.Jeschke PhD thesis, Eq.(2.40) and its description, Page 40
   type(TB_H_Fu), intent(in) :: TB ! all tight binding parameters
   ...
   if (i .EQ. j) then ! Onsite contributions, according to H.Jeschke, PhD thesis, Eq.(2.41), Page 40
      ...
      call dHopping_r(dtsx, dtsy, dtsz, d2tsx, d2tsy, d2tsz, i, j, k, Scell, NSC, atom_2, M_Vs, M_dVs, M_d2Vs, M_cos)
...
subroutine dHamilton_one_s(i, j, k, dHijx, dHijy, dHijz, Scell, NSC, atom_2, M_x1, M_xrr, M_Vs, M_dVs)
! Create a Hamiltonain-matrix, a block
! of which the total Hamiltonain is constructed
! See H.Jeschke PhD thesis, Eq.(2.40) and its description, Page 40
   integer, INTENT(IN) :: i, j, k, atom_2
   ...
   if (i .EQ. j) then ! Onsite contributions, according to H.Jeschke, PhD thesis, Eq.(2.41), Page 40
      ...
      ! For pairs of atoms, fill the hamiltonain with Hopping Integrals.
      ! Call the subroutine to calculate these hoppings: 
      call dHopping_s(dtsx, dtsy, dtsz, i, j, k, Scell, NSC, atom_2, M_x1, M_xrr, M_Vs, M_dVs)
      ! Then fill the hamiltonain with correspongin hopping integrals, as in
      ! H.Jeschke PhD thesis, Eq.(2.42), Page 40:
      do ki = 1,4
...
subroutine dHopping_r(dtsx, dtsy, dtsz,  d2tsx, d2tsy, d2tsz,  i, j, k, Scell, NSC, atom_2, M_Vs, M_dVs, M_d2Vs, M_cos)
...
subroutine dHopping_s(dtsx, dtsy, dtsz, i, j, k, Scell, NSC, atom_2, M_x1, M_xrr, M_Vs, M_dVs)
! subroutine making the derivatives of the hopping integrals
! dtsz, dtsy, dtsx --- the derivatives Hopping Integrals in Z,Y,X axis
! Xco --- atomic coordinates of all the atoms, 
! supce --- the supercell size
...
subroutine Attract_TB_Forces_Press_F(TB_Hamil, atoms, Scell, NSC, numpar, Aij)
   type(TB_H_Fu), dimension(:,:), intent(in) :: TB_Hamil ! all tight binding parameters
   type(Atom), dimension(:), intent(in) :: atoms	! array of atoms in the supercell
   type(Super_cell), dimension(:), intent(inout) :: Scell  ! supercell with all the atoms as one object
   integer, intent(in) :: NSC ! number of supercell
   ...
   if (numpar%p_const) then	! calculate this for P=const Parrinello-Rahman MD
      ...
      enddo ! i
      ! Collect information from all processes into the master process, and distribute the final arrays to all processes:
      error_part = 'Error in Attract_TB_Forces_Press_F:'
...
end subroutine Attract_TB_Forces_Press_F
   ...
   do i = 1,nat	! all atoms
      ...
      do atom_2 = 1,m ! do only for atoms close to that one
         ...
         if (j .GT. 0) then
            j4 = (j-1)*4
            !NumTB = numpar%El_num_ij(Scell(NSC)%MDatoms(i)%KOA, Scell(NSC)%MDatoms(j)%KOA)
            call dHamilton_one_Press(i, atom_2, Scell(NSC)%MDatoms, Scell, NSC, TB_Hamil, dHij1)
...
! See H.Jeschke PhD thesis, Eq.(2.40) and its description, Page 40
   ...
   integer, intent(in) :: NSC ! number of supercell
   type(Atom), dimension(:), intent(in) :: atoms	! array of atoms in the supercell
   type(TB_H_Fu), dimension(:,:), intent(in) :: TB	! all tight binding parameters   
...
! subroutine making the derivatives of the hopping integrals
   ...
   part1 = Vs(TB(KOA1,KOA2),2,r)
   part2 = Vs(TB(KOA2,KOA1),2,r)
   Vs2r = part1*TB(KOA1,KOA2)%C_a  + part2*TB(KOA2,KOA1)%C_a
   
   part1 = dVs(TB(KOA1,KOA2),2,r)
   part2 = dVs(TB(KOA2,KOA1),2,r)
   dVs2r = part1*TB(KOA1,KOA2)%C_a  + part2*TB(KOA2,KOA1)%C_a
   
   part1 = Vs(TB(KOA1,KOA2),3,r)
   part2 = Vs(TB(KOA2,KOA1),3,r)
   Vs3r =  part1*TB(KOA1,KOA2)%C_a  + part2*TB(KOA2,KOA1)%C_a
   
   part1 = dVs(TB(KOA1,KOA2),3,r)
   part2 = dVs(TB(KOA2,KOA1),3,r)
   dVs3r = part1*TB(KOA1,KOA2)%C_a  + part2*TB(KOA2,KOA1)%C_a
   
   part1 = Vs(TB(KOA1,KOA2),4,r)
   part2 = Vs(TB(KOA2,KOA1),4,r)
   Vs4r = part1*TB(KOA1,KOA2)%C_a  + part2*TB(KOA2,KOA1)%C_a
   
   part1 = dVs(TB(KOA1,KOA2),4,r)
   part2 = dVs(TB(KOA2,KOA1),4,r)
   dVs4r = part1*TB(KOA1,KOA2)%C_a + part2*TB(KOA2,KOA1)%C_a
  ...
  do k1 = 1,9 ! all the components of the h_alpha_beta(3,3):
   ...
   part1 = dVs(TB(KOA1,KOA2),1,r)
   part2 = dVs(TB(KOA2,KOA1),1,r)
   dtsx(k1,1,1) = (part1*TB(KOA1,KOA2)%C_a + part2*TB(KOA2,KOA1)%C_a)*drijdhgd ! ss
...
! Tight Binding Hamiltonian within Fu parametrization:
subroutine construct_TB_H_Fu(numpar, matter, TB_Hamil, Scell, NSC, Ha, Err)
   type(Numerics_param), intent(inout) :: numpar 	! all numerical parameters
...
end subroutine construct_TB_H_Fu



subroutine Complex_Hamil_tot_F(numpar, Scell, NSC, atoms, TB, Hij, CHij, ksx, ksy, ksz)
   type(Numerics_param), intent(inout) :: numpar 	! all numerical parameters
   type(Super_cell), dimension(:), intent(inout) :: Scell  ! supercell with all the atoms as one object
   integer, intent(in) :: NSC ! number of supercell
   type(Atom), dimension(:), intent(in) :: atoms	! array of atoms in the supercell
      ...
      do j = 1,nat	! all atoms
         ...
         do atom_2 = 0,m ! do only for atoms close to that one  
            ...
            if (i .GT. 0) then
               ...
               call Hamilton_one(Scell, NSC, i, j, atoms, TB, Hij1, x1=x, y1=y, z1=z, cell_x=cell_x, cell_y=cell_y, cell_z=cell_z)
...
end subroutine Complex_Hamil_tot_F
      ...
      do atom_2 = 0,m ! do only for atoms close to that one
         ...
         if (i .GT. 0) then ! if there really is a nearest neighbor
            ...
            if (numpar%optic_model .EQ. 3) then ! create matrix element:
               ...
               call Hamilton_one(Scell, NSC, i, j, Scell(NSC)%MDatoms, TB_Hamil, Hij1, x1=x, y1=y, z1=z) ! block-hamiltonian and the shortest distances
            else
               call Hamilton_one(Scell, NSC, i, j, Scell(NSC)%MDatoms, TB_Hamil, Hij1) ! this calles the block-hamiltonian
            endif
            do j1 = 1,4 ! all orbitals
               l = (j-1)*4+j1
               do i1 = 1,4 ! all orbitals
...
!                      write(*,'(i4,i4,e,e,e)') k,l, matter%PRRx(k,l), matter%PRRy(k,l), matter%PRRz(k,l)
   ...
   enddo ! i
   ! Collect information from all processes into the master process, and distribute the final arrays to all processes:
   error_part = 'Error in Fu: Hamil_tot:'
...
subroutine Hamilton_one(Scell, NSC, i, j, atoms, TB, Hij,  x1, y1, z1, sx1, sy1, sz1, cell_x, cell_y, cell_z)
   ...
   REAL(8), DIMENSION(4,4) :: ts_part1, ts_part2
   ...
   if (i .EQ. j) then ! Onsite contributions, according to H.Jeschke, PhD thesis, Eq.(2.41), Page 40
      ...
      KOA1 => Scell(1)%MDatoms(i)%KOA
!       KOA1 => Scell(1)%MDatoms(1)%KOA
!       KOA2 => Scell(1)%MDatoms(2)%KOA
      Hij(1,1) =  TB(KOA1,KOA1)%Es
!       Hij(1,1) =  TB(KOA1,KOA1)%Es*TB(KOA1,KOA1)%C_a + TB(KOA2,KOA2)%Es*TB(KOA2,KOA2)%C_a
      do ki = 2,4
         Hij(ki,ki) = TB(KOA1,KOA1)%Ep
!          Hij(ki,ki) = TB(KOA1,KOA1)%Ep*TB(KOA1,KOA1)%C_a + TB(KOA2,KOA2)%Ep*TB(KOA2,KOA2)%C_a
      enddo
      ...
      ! This version includes averagin inside of the subroutine:
      call Hopping(Scell, TB, i, j, ts, x, y, z, r)
      
      ! This version makes NO difference at all:
!        KOA1 => Scell(1)%MDatoms(i)%KOA
!        KOA2 => Scell(1)%MDatoms(j)%KOA
!        call Hopping_single(TB(KOA1,KOA2), ts_part1, x, y, z, r)
!        call Hopping_single(TB(KOA2,KOA1), ts_part2, x, y, z, r)
!        ts = ts_part1*TB(KOA1,KOA2)%C_a + ts_part2*TB(KOA2,KOA1)%C_a
...
subroutine Hopping_single(TB, ts, x, y, z, r0)
...
! x,y,z --- the distances between naighbour atoms we are analizing
   type(TB_H_Fu), intent(in), target :: TB	! all tight binding parameters
   ...
   nullify(rm)
end subroutine Hopping_single



subroutine Hopping(Scell, TB, i, j, ts, x, y, z, r0)
! subroutine making the hopping integrals
! ts --- the Hopping Integrals themselves
! x,y,z --- the distances between naighbour atoms we are analizing
   type(Super_cell), dimension(:), intent(in), target :: Scell  ! supercell with all the atoms as one object
   type(TB_H_Fu), dimension(:,:), intent(in), target :: TB	! all tight binding parameters
   integer, intent(in) :: i, j
   REAL(8), DIMENSION(4,4), INTENT(out) :: ts
   real(8), intent(in) :: x,y,z	! relative distances (projections) between the atoms
   real(8), intent(in), optional :: r0
   !=============================================
   real(8), pointer :: rm, rm2
   integer, pointer :: KOA1, KOA2
   real(8) r, r1, xr, yr, zr, Vsr(4)
   real(8) part1, part2
   if (present(r0)) then
   ...
   rm => TB(KOA1, KOA2)%rm ! cut-off radius
   rm2 => TB(KOA2, KOA1)%rm ! cut-off radius
   
   if (r <= max(rm,rm2)) then   ! these atoms do interact:
      ! All of the hopping integrals ts for each pair of atoms i and j,
      ! according to the Eqs.(2.23)-(2.29) from PhD Thesis of H.Jeschke, Page 38
      xr = x/r
      yr = y/r
      ...
      part1 = Vs(TB(KOA1,KOA2),1,r)	! calling the function given below
      part2 = Vs(TB(KOA2,KOA1),1,r)	! calling the function given below
      Vsr(1) = part1*TB(KOA1,KOA2)%C_a + part2*TB(KOA2,KOA1)%C_a
      
      part1 = Vs(TB(KOA1,KOA2),2,r)	! calling the function given below
      part2 = Vs(TB(KOA2,KOA1),2,r)	! calling the function given below
      Vsr(2) = part1*TB(KOA1,KOA2)%C_a + part2*TB(KOA2,KOA1)%C_a
      
      part1 = Vs(TB(KOA1,KOA2),3,r)	! calling the function given below
      part2 = Vs(TB(KOA2,KOA1),3,r)	! calling the function given below  
      Vsr(3) = part1*TB(KOA1,KOA2)%C_a + part2*TB(KOA2,KOA1)%C_a
      
      part1 = Vs(TB(KOA1,KOA2),4,r)	! calling the function given below
      part2 = Vs(TB(KOA2,KOA1),4,r)	! calling the function given below  
      Vsr(4) = part1*TB(KOA1,KOA2)%C_a + part2*TB(KOA2,KOA1)%C_a
   ...
   do i1 = 1, Scell(NSC)%Na
      ...
      do atom_2 = 1, m ! do only for atoms close to that one
         ...
         if (j1 .NE. i1) then
            ...
            part1 = phi(TB_Repuls(KOA1, KOA2),a_r)
            part2 = phi(TB_Repuls(KOA2, KOA1),a_r)
            b = b + (part1*TB_Repuls(KOA1, KOA2)%C_a + part2*TB_Repuls(KOA2, KOA1)%C_a)
...
!       a = a + fx(TB_Repuls(Scell(NSC)%MDatoms(i1)%KOA,Scell(NSC)%MDatoms(i1)%KOA),b) + TB_Repuls(Scell(NSC)%MDatoms(i1)%KOA,Scell(NSC)%MDatoms(i1)%KOA)%E0_TB
   enddo ! i1
   nullify(KOA1, KOA2)
end subroutine get_Erep_s_F


function fx(TB_Repuls,a_x) ! polinomial form, for repulsive potential
   ! uses Eq. 2.36, Page 40 in H.Jeschke PhD thesis.
   type(TB_Rep_Fu), intent(in), target :: TB_Repuls   ! parameters of the repulsive part of TB-H
...
subroutine dErdr_s_F(TB_Repuls, atoms, Scell, NSC, numpar) ! derivatives of the repulsive energy by s
   type(TB_Rep_Fu), dimension(:,:), intent(in)   :: TB_Repuls ! repulsive TB parameters
   type(Super_cell), dimension(:), intent(inout), target :: Scell  ! supercell with all the atoms as one object
   integer, intent(in) :: NSC ! number of supercell
   type(Atom), dimension(:), intent(in) :: atoms	! array of atoms in the supercell
   type(Numerics_param), intent(inout) :: numpar	! numerical parameters, including lists of earest neighbors
   ...
   real(8) part1, part2, d_part1, d_part2, frac
   ...
   !do ian = 1, n  ! Forces for all atoms
     ...
     do i1 = 1, n
         ...
         do atom_2 = 1,m ! do only for atoms close to that one
            ...
            if (j1 .GT. 0) then
                ...
                if ((j1 .NE. i1) .OR. ((dik-djk) .NE. 0)) then ! without it, it gives ERROR
                  ...
                  part1 = phi(TB_Repuls(KOA1, KOA2),a_r)
                  part2 = phi(TB_Repuls(KOA2, KOA1),a_r)
                  psi = psi + (part1*TB_Repuls(KOA1, KOA2)%C_a + part2*TB_Repuls(KOA2, KOA1)%C_a)
                  ...
                  d_part1 = dphi(TB_Repuls(KOA1, KOA2),a_r)
                  d_part2 = dphi(TB_Repuls(KOA2, KOA1),a_r)
                  b = d_part1*TB_Repuls(KOA1, KOA2)%C_a + d_part2*TB_Repuls(KOA2, KOA1)%C_a
         ...
         do j1 = 1, size(TB_Repuls,1)
            call get_fraction_of_given_sort(Scell, j1, frac)	! module "Atomic_tools"
            a = a + dfx(TB_Repuls(j1,j1),psi)*frac
         enddo
...
#else    ! OpenMP to use instead
   !$omp PARALLEL DO private(ian,i1,dpsi,psi,m,j1,x,y,z,a_r,dik,djk,x1,b,ddlta,b_delta,a,atom_2,NumTB, part1, part2, d_part1, d_part2, frac,  KOA1, KOA2)
   do ian = 1, n  ! Forces for all atoms
     Scell(NSC)%MDatoms(ian)%forces%rep(:) = 0.0d0 ! just to start with
     do i1 = 1, n
         dpsi = 0.0d0
...
!          part1 = dfx(TB_Repuls(1,1),psi)
!          part2 = dfx(TB_Repuls(2,2),psi)
!          a = part1*TB_Repuls(1,1)%C_a + part2*TB_Repuls(2,2)%C_a
         ! Sorts of atoms:
         a = 0.0d0
         do j1 = 1, size(TB_Repuls,1)
            call get_fraction_of_given_sort(Scell, j1, frac)	! module "Atomic_tools"
...
subroutine dErdr_Pressure_s_F(TB_Repuls, atoms, Scell, NSC, numpar) ! derivatives of the repulsive energy by h
   type(TB_Rep_Fu), dimension(:,:), intent(in) :: TB_Repuls ! repulsive TB parameters
   type(Super_cell), dimension(:), intent(inout) :: Scell  ! supercell with all the atoms as one object
   integer, intent(in) :: NSC ! number of supercell
   type(Atom), dimension(:), intent(in) :: atoms ! array of atoms in the supercell
   ...
   integer i,j,k,l, i1, i2, i3, ik, m, atom_2, n, NumTB, KOA1, KOA2
   real(8) x,y,z,sx,sy,sz,r, rcur(3), scur(3)
   real(8) df_psy, psi, dpsy, part1, part2, d_part1, d_part2, frac
      ...
      do i = 1, n ! Forces from all atoms
         ...
         do atom_2 = 1,m ! do only for atoms close to that one
            ...
            if ((i .NE. j) .AND. (j .GT. 0)) then
               ...
               KOA1 = Scell(NSC)%MDatoms(i)%KOA
               KOA2 = Scell(NSC)%MDatoms(j)%KOA
               part1 = phi(TB_Repuls(KOA1,KOA2),r)
               part2 = phi(TB_Repuls(KOA2,KOA1),r)
               psi = psi + (part1*TB_Repuls(KOA1,KOA2)%C_a + part2*TB_Repuls(KOA2,KOA1)%C_a)                  
!                psi = psi + phi(TB_Repuls(Scell(NSC)%MDatoms(j)%KOA, Scell(NSC)%MDatoms(i)%KOA),r) ! Eq.(F21), p.147 H.Jeschke PhD Thesis

               d_part1 = dphi(TB_Repuls(KOA1,KOA2),r)
               d_part2 = dphi(TB_Repuls(KOA2,KOA1),r)
               dpsy = d_part1*TB_Repuls(KOA1,KOA2)%C_a + d_part2*TB_Repuls(KOA2,KOA1)%C_a
...
!          df_psy = dfx(TB_Repuls(Scell(NSC)%MDatoms(i)%KOA, Scell(NSC)%MDatoms(i)%KOA),psi)
         ...
         do j = 1, size(TB_Repuls,1)
            call get_fraction_of_given_sort(Scell, j, frac)	! module "Atomic_tools"
            df_psy = df_psy + dfx(TB_Repuls(j,j),psi)*frac
         enddo
...
end subroutine dErdr_Pressure_s_F
