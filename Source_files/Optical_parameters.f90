...
use Nonadiabatic, only : get_Mij2
...
subroutine get_Onsager_ABC_dynamic(numpar, Scell, NSC, Mij, eta, A, B, C)
   ...
   type(Super_cell), dimension(:), intent(in) :: Scell  ! supercell with all the atoms as one object
   integer, intent(in) :: NSC ! number of supercell
   real(8), dimension(:,:), intent(in) :: Mij  ! dynamical coupling matrix
   real(8), intent(in) :: eta ! [eV]
   real(8), intent(inout) :: A, B, C
   !---------------------
   real(8) :: prec, delta, Eij, P2, E_mu, A_cur, B_cur, C_cur, f_nm, f_delt, coef, coef_inv, dt_small, v2
   integer :: n, m, i, Nsiz, N_orb, N_at
   ...
   !$omp PARALLEL private(n, m, Eij, delta, P2, E_mu, A_cur, B_cur, C_cur, f_nm, f_delt, i, v2)
   !$omp do schedule(dynamic)  reduction( + : A, B, C)
   do n = 1, Nsiz ! all energy points
      i = 1 + (n-1)/N_orb  ! atomic index
      v2 = SUM(Scell(NSC)%MDatoms(i)%V(:)*Scell(NSC)%MDatoms(i)%V(:))*1d10  ! [(m/s)^2]
      ...
      do m = 1, Nsiz
         ...
         if ( (n /= m) .and. (abs(Eij) > prec) .and. (abs(delta) > prec) ) then   ! nondegenerate levels
            ! Average momentum operator:
            call get_Mij2(n, m, numpar%NA_kind, Scell(NSC)%Mij, dt_small, Scell(NSC)%Ei, Scell(NSC)%Ei0, &
                           coef, coef_inv, P2)   ! module "Nonadiabatic"
            P2 = P2 * g_h**2 / v2   ! [ (kg*m/s) ^2 ]
