...
def main():
    ...
    peaks_files = glob.glob("OUTPUT_diffraction_peaks*.dat")

    if not powder_files or not peaks_files:
        print("No input files found in the current directory.")
        input("Press Enter to exit...")
        return

    # Use the first matching peaks file
    peak_info = parse_peaks_header(peaks_files[0])

    # Analyze each powder file
    for pf in powder_files:
        print(f"Analyzing {pf}...")
        df = analyze_run(pf, peak_info)
        out_name = os.path.splitext(pf)[0] + "_fit_results.csv"
