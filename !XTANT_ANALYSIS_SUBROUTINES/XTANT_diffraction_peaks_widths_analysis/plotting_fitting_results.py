...
# --- Load fitted results ---
df = pd.read_csv("OUTPUT_diffraction_powder_fit_results.csv")  # adjust filename
...
def animate_peaks_with_raw(df, blocks, out_file="peak_evolution_with_raw.mp4"):
    fig, ax = plt.subplots(figsize=(8, 6))

    times = sorted(df["time_fs"].unique())
    hkl_list = df["hkl"].unique()

    # Prepare line objects for each hkl fit
    fit_lines = {}
    for hkl in hkl_list:
        line, = ax.plot([], [], label=f"fit {hkl}")
        fit_lines[hkl] = line

    # Raw data line
    raw_line, = ax.plot([], [], "o", ms=2, color="0.5", label="raw data")

    ax.set_xlim(0, 180)   # ✅ fix X axis to 0–180 deg
...
def plot_integrated_areas(df, out_file="peak_integrated_areas_vs_time.png"):
...
def plot_normalized_areas(df, out_file="peak_normalized_areas_vs_time.png",
    ...
    for hkl in df["hkl"].unique():
        ...
        if initial_area != 0:
            subset["normalized_area"] = subset["integrated_area"] / initial_area
        else:
            subset["normalized_area"] = 0.0
        lw = 0.2 if initial_area < min_area_threshold else 1.5
        alpha = 0.6 if initial_area < min_area_threshold else 1.0
        ax.plot(subset["time_fs"], subset["normalized_area"],
                label=hkl, lw=lw, alpha=alpha)
    ax.set_xlabel("Time (fs)")
...
if __name__ == "__main__":
    plot_integrated_areas(df, out_file="peak_integrated_areas_vs_time.png")
    plot_normalized_areas(df, out_file="peak_normalized_areas_vs_time.png")
    animate_peaks_with_raw(df, blocks, out_file="peak_evolution_with_raw.mp4")
